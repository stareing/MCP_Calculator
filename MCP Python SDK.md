...
Model Context Protocol 允许应用程序以标准化的方式为 LLM 提供上下文，将提供上下文的关注点与实际的 LLM 交互分离。这个 Python SDK 实现了完整的 MCP 规范，让你能够轻松地：
...
你可以通过运行以下命令将此服务器安装到 [Claude 桌面版](https://claude.ai/download) 并立即与之交互：
```bash
